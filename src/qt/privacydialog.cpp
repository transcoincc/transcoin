...
#include "ztranscoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTRNS ought to be enough for anybody." - Bill Gates, 2017
    ui->zTRNSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTRNSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTRNS->setEnabled(false);
        ui->pushButtonMintzTRNS->setToolTip(tr("zTRNS is currently disabled due to maintenance."));

        ui->pushButtonSpendzTRNS->setEnabled(false);
        ui->pushButtonSpendzTRNS->setToolTip(tr("zTRNS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTRNSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTRNS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTRNS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTRNS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTRNS in ") + 
...
void PrivacyDialog::on_pushButtonSpendzTRNS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTRNS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTRNS
        sendzTRNS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTRNS
    sendzTRNS();
}

void PrivacyDialog::on_pushButtonZTransCoinControl_clicked()
{
    ZTransCoinControlDialog* zTransCoinControl = new ZTransCoinControlDialog(this);
    zTransCoinControl->setModel(walletModel);
    zTransCoinControl->exec();
}

void PrivacyDialog::setZTransCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTransCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTRNS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid TransCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTRNSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTRNS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TRNS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTRNS</b>";
    ...
    // use mints from zTransCoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTransCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTransCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear ztranscoin selector in case it was used
    ZTransCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTransCoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TransCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTransCoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTRNS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTRNS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTRNS "));
    ui->labelzTRNSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTRNSSyncStatus->setVisible(fShow);
}
