...
#include "ui_ztranscoincontroldialog.h"
...
std::list<std::string> ZTransCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTransCoinControlDialog::listMints;

ZTransCoinControlDialog::ZTransCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTransCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTransCoinControlDialog::~ZTransCoinControlDialog()
{
    delete ui;
}

void ZTransCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTransCoinControlDialog::updateList()
{
...
void ZTransCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTransCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTransCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTransCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTransCoinControlDialog::GetSelectedMints()
{
...
void ZTransCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
